defmodule Absinthe.GraphqlWS.ClientTest do
  ...
  setup do
    [caller_pid: 222, gun_pid: 111, gun_stream_ref: make_ref()]
  end
  ...
  describe "start" do
    ...
    test "returns an error on multiple connection_init" do
      ...
      ExUnit.CaptureLog.capture_log(fn ->
        assert {:error, 4429, "Too many initialisation requests"} =
                 Client.push(client, %{type: "connection_init", payload: %{}})
  ...
  describe "handle_call" do
    test "push connection init stores the caller with key connection_init", %{
      caller_pid: caller_pid,
      gun_pid: gun_pid,
      gun_stream_ref: gun_stream_ref
    } do
      {:noreply, state} =
        Client.handle_call({:push, %{type: "connection_init"}}, caller_pid, %{
          queries: %{},
          listeners: %{},
          gun: gun_pid,
    ...
    test "push message with id stores the caller on the id", %{
      caller_pid: caller_pid,
      gun_pid: gun_pid,
      gun_stream_ref: gun_stream_ref
    } do
      ...
      {:noreply, state} =
        Client.handle_call({:push, %{id: message_id}}, caller_pid, %{
          queries: %{},
          gun: gun_pid,
          transport: FakeTransport,
          gun_stream_ref: gun_stream_ref
  ...
  describe "handle_info" do
    ...
    test "DOWN stops the server", %{caller_pid: caller_pid, state: state} do
      assert {:stop, "end-of-test", ^state} =
               Client.handle_info({:DOWN, "fake-ref", :process, caller_pid, "end-of-test"}, state)
    end

    test "gun_error stops the server", %{caller_pid: caller_pid, state: state} do
      assert {:stop, "end-of-test", ^state} =
               Client.handle_info({:gun_error, caller_pid, "stream-ref", "end-of-test"}, state)
    end

    test "gun_ws handles completed requests and removes them from the query map", %{
      caller_pid: caller_pid
    } do
    ...
    test "gun_ws handles completed subscription feeds and removes them from the listeners map", %{
      caller_pid: caller_pid
    } do
      payload = Jason.encode!(%{id: "easy-message-id", type: "complete"})
    ...
    test "gun_ws forwards requested info to the process named in the query map", %{
      caller_pid: caller_pid
    } do
      ...
      assert(
        {:noreply, ^state} =
          Client.handle_info(
            {:gun_ws, caller_pid, "stream-ref", {:text, Jason.encode!(message)}},
            state
          )
      )
    ...
    test "gun_ws forwards requested info to the subscribers named in the listeners map", %{
      caller_pid: caller_pid
    } do
    ...
    test "gun_ws forwards connection_ack to the process requesting connection_init", %{
      caller_pid: caller_pid
    } do
      message = %{"type" => "connection_ack"}

      state = %{
        queries: %{"connection_init" => {self(), :tag}, "other-message-id" => 501},
        listeners: %{}
      }
  ...
  describe "query" do
    ...
    test "it returns the data you asked for", %{client: client} do
      assert(
        {:ok,
         %{
           "data" => %{"things" => [%{"id" => 1, "name" => "one"}, %{"id" => 2, "name" => "two"}]}
